# Historical Basic Specifications
...
- **Enrichment engines** — `core/planet_strength.*`, `core/house_state.*`, `core/lagna_role.*` : compute dignity, shadbala, avasthas, functional roles.
  - Note: `AstroState` models the Lagna (ascendant) as explicit fields `lagna_sign` and `lagna_degree` rather than an `Ascendant` planet node. Planet lists contain only planetary bodies; normalizer converts provider-specific house indices (e.g., Surya's Aries-based numbering) into lagna-relative house numbers so house 1 == lagna for whole-sign charts.
  - DerivedState: the engine now builds a consolidated `derived` bundle (planets, houses, functional_roles, relationships) validated by Pydantic models (`systems/Parasara/engine/derived/models.py`) so rule evaluation consumes canonical, precomputed facts.
  - Functional-role tables: per-lagna YAML overrides live under `systems/Parasara/enrichment_tables/functional_roles/` and are used by the functional-role engine for SME-tuned role mapping.
  - Strength components: `planet_strengths` includes `strength_components` (dignity_bonus, combust_penalty, retro_bonus, temp_friend_bonus, varga_bonus) for explainability and diagnostics.
...
### Engine capabilities and M1 contract

Include an explicit `capabilities` section in the top-level output to signal which features the engine currently supports. Example:
...
"capabilities": {
  ...
  "vimshottari": true,
  "transits": false,
  "advanced_shadbala": false,
  "advanced_yogas": false,
  "d10": false
}
```

This lets clients adapt to progressive feature availability and keeps backwards compatibility.
...
- `explainability`: `indicators_legend`, `scoring_formula`, `conflict_resolution_policy`.
  - `indicators`: M1 interpreters populate `indicators` as a list of objects with at minimum `rule_id`, `contribution`, and `evidence` (optionally `context` and `provenance`). See `systems/Parasara/engine/interpreters/career.py` for the M1 shape.
Include example `fixtures/output_golden_chart_01.json`.

### M1 Output stability

For M1 we will freeze the high-level JSON sections and avoid structural breaking changes. The top-level sections that must remain stable are:
...
Additions are allowed (new fields) but do not remove or rename the above sections during M1.x.
...
Rules must produce structured `RuleMatch` objects for auditing and calibration. A recommended M1 schema:
...
RuleMatch {
  ...
  matched: boolean,
  priority: int | null,
  context: object,            # original rule or reference
  base_score: float,          # canonical score defined on rule
  adjusted_score: float,      # post-rule runtime adjustments
  confidence: float,          # rule-level confidence (0..1)
  evidence: object,           # evaluator evidence blob
  trace_id: string,           # request-scoped trace id
  provenance: {file: string, line: int} | null,
  evaluation_time_ms: int | null
}
```

Interpreters must consume `RuleMatch` objects (or a typed equivalent) rather than ad-hoc booleans so that every decision is auditable and traceable.
Governance:
...
- Backtesting harness to compute `historical_accuracy`.
 - CI & snapshot testing: a GitHub Actions workflow runs `pytest` on push/PR; golden snapshots for career domain are stored under `systems/Parasara/tests/fixtures/` and compared during tests.
