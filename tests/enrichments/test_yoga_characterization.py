...
def test_repeat_evaluation_is_fully_deterministic_and_does_not_drift_registries():
    astro = prepared_surya_test()
    registry_before = deepcopy(RULE_REGISTRY)
    predicate_ids_before = get_production_registry().exposed_ids()
    ...
    assert get_production_registry().exposed_ids() == predicate_ids_before
